...
- 모델 다운로드 안내: 매니페스트에서 받은 모델 이름·용량(E4B 약 3GB)·저장 위치(브라우저 캐시)를 표시. "동의하고 다운로드" 버튼.
...
- **E4B 자원**: 웹용 파일 약 3GB, GPU 메모리 여유 필요. D-6에 데모 노트북에서 로드 확인이 첫 작업이다. 안 되면 `fallback`(E2B)으로 간다.
- **Gemma 4 웹용 모델과 템플릿 실체**: 2026-09-15 검증 완료. 템플릿·URL 정확, E4B 웹용 실제 크기 약 3.0GB. 상세와 프론트 영향(LiteRT-LM JS API 권장)은 `docs/model-verification-2026-09-15.md`.
